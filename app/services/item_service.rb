class ItemService
  class << self
    def create_item_from_upc(upc)
      ...
      custom_measurement_unit_names = data[:measurement_units] - MeasurementUnit.standard.keys
      custom_measurement_units = MeasurementUnit.where(name: custom_measurement_unit_names).to_a
      new_measurement_unit_names =
        custom_measurement_unit_names - custom_measurement_units.map(&:name)

      ApplicationRecord.transaction do
        new_measurement_unit_names.each do |new_measurement_unit_name|
          custom_measurement_units << MeasurementUnit.create!(name: new_measurement_unit_name)
        end
        ...
        custom_measurement_units.each do |custom_measurement_unit|
          ItemMeasurementUnit.create!(
            grams: Edamam.get_grams_per_measurement_unit(
              ...
              measurement_name: custom_measurement_unit.name
            ),
            item: item,
